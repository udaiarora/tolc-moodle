    ...
    if ($users && $posts) {
        ...
        foreach ($users as $userto) {
            ...
            foreach ($posts as $pid => $post) {
                ...
                if($post->alias) {
                    $smallmessagestrings->user = aliasname($userfrom);
                }
                else
                {
...
function forum_make_mail_text($course, $cm, $forum, $discussion, $post, $userfrom, $userto, $bare = false) {
    ...
    $by = New stdClass;
    if($post->alias) {
        $by->name = aliasname($userfrom, $viewfullnames);
    }
    else {
...
function forum_print_recent_activity($course, $viewfullnames, $timestart) {
    ...
    foreach ($printposts as $post) {
        ...
        if($post->alias) {
            echo '<div class="name">'.aliasname($post, $viewfullnames).'</div>';
        }
        else {
            echo '<div class="name">'.fullname($post, $viewfullnames).'</div>';
        }
        echo '</div>';
...
function forum_make_mail_post($course, $cm, $forum, $discussion, $post, $userfrom, $userto,
    ...
    $fullname = ($post->alias)?aliasname($userfrom, $viewfullnames):fullname($userfrom, $viewfullnames);
    $by = new stdClass();
    if($post->alias)
    {
        $by->name = $fullname;
...
function forum_print_post($post, $discussion, $forum, &$cm, $course, $ownpost=false, $reply=false, $link=false,
    ...
    $alias = $post->alias;
    if($alias) {
        $postuser->fullname    = aliasname($postuser, $cm->cache->caps['moodle/site:viewfullnames']);
    }
    else {
...
$disc = get_firstlast_alias($post);

    echo '<td class="author">';
    // User name
    if($disc->firstalias) {
        $fullname = aliasname($postuser, has_capability('moodle/site:viewfullnames', $modcontext));
        echo $fullname;
    }
    else {
    ...
    if($disc->lastalias)
    {
        echo aliasname($usermodified).'<br />';
    }
    else {
...
function forum_add_new_post($post, $mform, &$message) {
    ...
    $post->userid     = $USER->id;
    $post->attachment = "";
    $post->alias = !empty($post->alias);

    $post->id = $DB->insert_record("forum_posts", $post);
    $post->message = file_save_draft_area_files($post->itemid, $context->id, 'mod_forum', 'post', $post->id,
            mod_forum_post_form::editor_options($context, null), $post->message);
    $DB->set_field('forum_posts', 'message', $post->message, array('id'=>$post->id));
    $DB->set_field('forum_posts', 'alias', $post->alias, array('id'=>$post->id));
    ...
    $DB->set_field("forum_discussions", "lastalias", $post->alias, array("id" => $post->discussion));
...
function forum_update_post($post, $mform, &$message) {
    ...
    $DB->set_field('forum_posts', 'message', $post->message, array('id'=>$post->id));
    $DB->set_field('forum_posts', 'alias', !empty($post->alias), array('id'=>$post->id));

    $DB->update_record('forum_discussions', $discussion);
    if(!$post->parent) {
        $DB->set_field('forum_discussions', 'firstalias', !empty($post->alias), array('id'=>$post->discussion));
    }
...
function forum_add_discussion($discussion, $mform=null, $unused=null, $userid=null) {
    ...
    $post->alias         = !empty($discussion->alias);
    ...
    $DB->set_field("forum_discussions", "firstalias", $post->alias, array("id" => $post->discussion));
    $DB->set_field("forum_discussions", "lastalias", $post->alias, array("id" => $post->discussion));
...
function get_firstlast_alias($post) {
    global $DB;
    $discussion = $DB->get_record('forum_discussions', array('id'=> $post->discussion));
    return $discussion;    
}
    ...
    if (!empty($posts[$parent->id]->children)) {
        ...
        foreach ($posts as $post) {
            ...
            if ($depth > 0) {
                ...
                $by = new stdClass();
                                if($post->alias) {
                    $by->name = aliasname($post, $canviewfullnames);
                }
                else {
...
function forum_print_recent_mod_activity($activity, $courseid, $detail, $modnames, $viewfullnames) {
    ...
    echo '<div class="user">';
 	    if($post->alias) {
        $fullname = aliasname($activity->user, $viewfullnames);
    }
    else {
